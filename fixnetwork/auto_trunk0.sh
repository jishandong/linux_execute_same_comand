...
# -b 参数创建trunk+bridge接口 并创建私网IP子接口

# -t 参数只创建trunk接口 并创建私网IP子接口
...
mv -f $confDir/ifcfg-trunk0 $backConfDir

cat >> $confDir/ifcfg-trunk0 <<EOF
...
options trunk0 miimon=100 mode=1
...
mv -f $confDir/ifcfg-trunk0 $backConfDir 1>/dev/null 2>&1
